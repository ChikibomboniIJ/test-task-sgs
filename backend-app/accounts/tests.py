...
class AccountsViewsTest(APITestCase):
    ...
    def test_login_user(self):
        response = self.client.post(reverse('login'), {"username": "user", "password": "1234567890!"})
    ...
    def test_change_password(self):
        ...
        response = self.client.post(reverse("change-password"), data = {
            ...
            "new_password_confirmed": "1234567890!!"
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_logout_invalid_refresh_token(self):
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")
        response = self.client.post(reverse("logout"), {"refresh": "invalid_refresh_token"})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("error", response.data)
